...
if os.path.isfile("results_feiface_densenet.txt"):
    os.remove("results_feiface_densenet.txt")

def store_feiface_models():
    global r
    global img2vec
    
    for person in range(1, 201):
        person = "s" + str(person)
        #for face in [1, 3, 5, 7, 9, 11, 13]:
        #for face in [2, 4, 6, 8, 10, 12, 14]:
    ...
    for person in range(1, 201):
        ...
        for face in range(8, 15):
            ...
            if (person == found):
                ...
                f = open("results_feiface_densenet.txt", "a")
                f.write("Face not found:" + person + "/" + str(face) + "\n")


    print(success/1400*100)
...
store_feiface_models()
start_test = time.perf_counter()

test_feiface_models_vect()
